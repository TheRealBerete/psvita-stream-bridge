"""
Construit data/channels.json, data/categories.json, data/countries.json et
data/languages.json a partir de l'API publique iptv-org (channels.json,
categories.json, streams.json, countries.json, feeds.json, languages.json) :
  ...
  - associe chaque chaine a ses categories (ids -> noms lisibles) ET a ses
    langues de diffusion (via feeds.json, qui lie channel -> feed -> langues ;
    channels.json lui-meme ne porte pas cette info)
  - ne garde que les pays/langues qui ont au moins 1 chaine valide
...
def build_language_index(feeds: list[dict]) -> dict[str, list[str]]:
    """chan_id -> liste de codes langue (ISO 639-3), a partir du feed principal
    (is_main=true) de la chaine, ou a defaut le premier feed avec une langue."""
    main_langs: dict[str, list[str]] = {}
    any_langs: dict[str, list[str]] = {}
    for f in feeds:
        chan_id = f.get("channel")
        langs = f.get("languages")
        if not chan_id or not langs:
            continue
        any_langs.setdefault(chan_id, langs)
        if f.get("is_main"):
            main_langs.setdefault(chan_id, langs)
    return {**any_langs, **main_langs}
...
def main():
    ...
    for c in channels:
        ...
        langs = lang_by_channel.get(c["id"]) or ["und"]  # "und" = langue indeterminee (norme ISO 639-2)
        all_channels.append({
            "id": c["id"],
            "name": c["name"],
            "country": c["country"],
    ...
    lang_names["und"] = "Langue indeterminee"
    out_languages = {code: lang_names.get(code, code) for code in used_lang_codes}
    with (DATA_DIR / "languages.json").open("w", encoding="utf-8") as f:
        json.dump(out_languages, f, ensure_ascii=False, indent=2)

    print(f"{len(all_channels)} chaines valides, {len(out_countries)} pays, "
          f"{len(out_categories)} categories, {len(out_languages)} langues.")
